...
@master_only
def copy_framework_configs(cfg, args):
    name = args.config.split('/')[2]
    src_path = os.path.join('projects', name, 'mmdet3d_plugin', 'models', 'detectors', cfg.model.type + '.py')
    dst_path = os.path.join(cfg.work_dir, 'framework.py')
...
def main():
    ...
    if hasattr(model, 'teacher'): 
        ...
        model_parameters = model_parameters - teacher_parameters
    print('MODEL TOTAL PARAMETERS = %d'%(model_parameters))
